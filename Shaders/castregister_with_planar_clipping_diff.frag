...
//        bool in_slice = false;
        for (float i=0.0; i < steps; i+=1.0)
        {
            ...
            if(!is_grayscale)
            {
                ref_tf_color = vec4(texture(ref_volumetexture, current_position).rgb, 1.0);
//                tex_coord = vec4(current_position, 0);

//                if(noise_threshold > 0)
                if(state_noise_threshold)
                {
                    if((ref_tf_color.r + ref_tf_color.g + ref_tf_color.b) > 0.0)
//                        ref_tf_color = vec4(0.0, 0.0, 0.0, 0.0);
                        first = true;
                    else
//                        first = true;
                        ref_tf_color = vec4(0.0, 0.0, 0.0, 0.0);
...
//                    ref_eq_density = ref_density;
                    ref_tf_color = vec4(ref_density, ref_density, ref_density, 1.0);
                    if(use_transfer_function)
                        ref_tf_color = texture(ref_transferfunction, ref_density); // reference voxel color
...
//                fusion = mix(float_tf_color, vec4(0.f, 0.f, 0.f, 1.f), blending_factor);
